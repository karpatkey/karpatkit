...
class AllAbis:
    LiquidityPool = ContractAbi(abi_path=current_dir() / "liquidity_pool.json", name="liquidity_pool")
    LiquidityPoolToken = ContractAbi(abi_path=current_dir() / "liquidity_pool_token.json", name="liquidity_pool_token")
...
class EthereumContractSpecs:
    GbpPriceFeed = ContractSpec(
        ...
        abi_path=current_dir() / "price_feed.json",
        name="price_feed",
    )
